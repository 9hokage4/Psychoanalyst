...
class BarChartWidget(QWidget):
    ...
    def _plot_all_scales_for_group(self):
        """Столбцы по всем шкалам для выбранной группы/курса"""
        group_name = self.group_combo.currentText()
        ...
        bar_width = 0.8 / len(self.level_order)
        x = range(len(scales))
        colors = ["#66BB6A", "#FFA726", "#42A5F5", "#EF5350"][:len(self.level_order)]

        for i, level_key in enumerate(self.level_order):
            offset = (i - len(self.level_order) / 2 + 0.5) * bar_width
            ax.bar([p + offset for p in x], levels_data[level_key], bar_width,
    ...
    def _plot_by_group(self, scale_name):
        """x = Уровни для конкретной группы и шкалы"""
        group_name = self.group_combo.currentText()
        parsed = self._get_parsed_data(group_name)
        scale_data = parsed.get(scale_name, {})
        ...
        ax.bar(levels, percents, color=colors[:len(levels)])
        ax.set_xlabel('Уровни', fontsize=12)
        ax.set_ylabel('Процент (%)', fontsize=12)

        context = f"{self.scope_type.replace('Результат по ', '')}: {group_name}" if self.scope_type != "Результат по всем респондентам" else "Все респонденты"
    ...
    def _plot_all_scales(self):
        ...
        ax.set_xticks(x)
        ax.set_xticklabels(scales, rotation=45, ha='right')
        ax.legend(title="Уровни по всем респонентам", loc='upper right')
        ax.grid(axis='y', alpha=0.3)
        self.figure.tight_layout()
