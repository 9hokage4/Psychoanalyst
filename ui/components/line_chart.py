...
class LineChartWidget(QWidget):
    ...
    def set_data(self, charts_data: dict, level_order: list, level_ru: dict):
        ...
        if "mean_scores" in charts_data and not charts_data["mean_scores"].empty:
            self.available_scales = charts_data["mean_scores"]["Название шкалы"].tolist()
        else:
            self.available_scales = []
    ...
    def _on_scale_changed(self, scale_name: str):
        ...
        self._plot_line(scale_name)

    def _plot_line(self, scale_name: str):
        """Строит линейный график средних баллов по группам/курсам"""
        ...
        # Строим линии для каждого уровня
        x = range(len(groups))
        ...
        ax.set_title(f'Распределение уровней по группам: {scale_name}', fontsize=14, fontweight='bold', pad=15)
        ax.set_xticks(x)
